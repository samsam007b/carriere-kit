...
case "$state" in
  *'"workspace": false'*)
    ...
    echo "FIRST RUN, permissions: if this session asks the user to approve actions one by one, bypass mode did not start from the project settings. Offer \`python3 claude-setup/install.py --apply bypass\` once (it backs up their settings, then they restart Claude Code), or launching with \`claude --permission-mode bypassPermissions\`."
    ;;
  ...
  *)
    if [ -f tools/tracker.py ] && [ -f workspace/tracker.md ]; then
      fu=$(python3 tools/tracker.py followups 2>/dev/null | head -20)
